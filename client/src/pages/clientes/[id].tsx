        ...
        <TabsContent value="onboarding" className="space-y-6">
          <div className="grid gap-4">
            {getFollowUpStages().map((stage, index) => (
              <Card 
                key={stage.id} 
                className={`transition-all hover:shadow-md border-l-4 ${getPriorityColor(stage.priority, stage.isOverdue)}`}
              >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="text-center p-4 bg-red-50 rounded-lg border border-red-200">
                  <div className="text-2xl font-bold text-red-600">
                    {getFollowUpStages().filter(s => s.isOverdue).length}
                  </div>
                  <div className="text-sm text-red-600 font-medium">Atrasadas</div>
                </div>
                <div className="text-center p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                  <div className="text-2xl font-bold text-yellow-600">
                    {getFollowUpStages().filter(s => !s.isOverdue && dayjs().isAfter(s.targetDate.subtract(7, 'day'))).length}
                  </div>
                  <div className="text-sm text-yellow-600 font-medium">Próximas (7 dias)</div>
                </div>
                <div className="text-center p-4 bg-green-50 rounded-lg border border-green-200">
                  <div className="text-2xl font-bold text-green-600">
                    {getFollowUpStages().filter(s => !s.isOverdue && !dayjs().isAfter(s.targetDate.subtract(7, 'day'))).length}
                  </div>
                  <div className="text-sm text-green-600 font-medium">Futuras</div>
                </div>
              </div>
            </CardContent>
          </Card>
